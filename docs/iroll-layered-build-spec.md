...
- Irollfile 只保留三条核心指令，所有数据操作统一通过 SQL 完成
...
Irollfile 只保留三条指令：**FROM、MIGRATE、COPY**。
...
无 FROM 的 Irollfile 为基础层构建，此时创建空的 ai_roll.db 和 Resources/ 目录。

**Step 2：MIGRATE — 执行 SQL**

读取 Irollfile 同目录下的 SQL 文件，在构建目录的 `ai_roll.db` 上执行。
...
```bash
logos build -f Irollfile -t my-python-agent
```
...
```bash
logos build -f base-agent/Irollfile -t base-agent
```
...
```bash
logos build -f python-expert/Irollfile -t python-expert
```
...
```bash
logos build -f my-project/Irollfile -t my-project
```
...
| 新增命令 | 说明 |
|----------|------|
| `logos build -f Irollfile -t <name>` | 分层构建 |
...
- **Irollfile 只有三条指令**：FROM / MIGRATE / COPY，不引入更多指令
