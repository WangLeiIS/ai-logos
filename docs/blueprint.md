...
| Docker | Logos | 说明 |
|--------|-------|------|
| Dockerfile | Irollfile | 定义如何构建，指令式（FROM / MIGRATE / MIGRATE OUTER / COPY） |
...
| Volume | roll-inner.db + roll-outer.db + Resources/ | 持久化数据 |
| Layer | 构建历史 | 分层叠加，可追溯 |
...
Irollfile ──logos build──→ .iroll ──logos load──→ ~/.iroll/
                                                  ↓
                                          logos page new → Page
                                                  ↓
                                          对话中读写 context / memory
                                          对话结束状态保留在 roll-outer.db
```
...
| 维度 | 模块 | 类比 | 存储位置 |
|------|------|------|----------|
| 性格 | dna | 价值观 | roll-inner.db `dna` 表 |
| 习惯 | loop | 条件反射 | loop 种子在 roll-inner.db；loop_runs 在按 cwd 的 roll-outer.db |
| 经历 | memory | 日记 | 按 cwd 的 roll-outer.db `memory` 表；`forget` 尚未实现 |
| 知识 | book | 书架 | Resources/books/（Parquet + 清单） |
| 能力 | skill | 手艺 | Resources/skills/；已实现：构建时自动发现/校验/注册，`skill list`/`skill show` 查询 |
...
```
Irollfile + 资源文件 ──logos roll build──→ .iroll 包
```

Irollfile 共四条指令：`FROM`（继承基础层）、`MIGRATE`（执行 SQL）、`MIGRATE OUTER`（执行 outer 模板 SQL）、`COPY`（复制资源）。构建完成后会自动校验并注册 book 与 skill。

构建产出的 `.iroll` 包就是 agent 的「出厂设置」— 包含初始的人格（dna）、习惯（loop）、知识（book）以及已注册的 skill 元数据，但还没有任何记忆。
...
2. logos page get --cwd .           → 读取解析后的 context
3. 按照 context 中的指令行事
```
...
- `skills`：我能调用的能力列表（构建时已注册；`skill list`/`skill show` 按需查询，仅 name + description）
...
- context 变化时 → `logos page set <path> <value>`（或 `--content` 整体替换）更新上下文
...
每次构建都有历史记录（history 表），可以追溯演进过程。`.iroll` 内层蓝图（dna、loop 种子、book、skill、history）通过 `FROM` 跨版本继承，使 agent 升级后人格与能力得以延续；而 memory 与 loop_runs 落在按工作目录复制的 outer 模板及其运行实例中，并不随 `.iroll` 版本流转，由各工作目录自行保留。
...
```
                         构建
                          │
                     .iroll 包
                  (roll-inner.db + roll-outer.db + Resources/ + layer.json)
                          │
                    logos roll load
                          │
                     ~/.iroll/<name>/
...
- **Skill 已实现。** 构建时自动发现/校验/注册，运行时通过 `skill list`/`skill show` 查询能力元数据。
