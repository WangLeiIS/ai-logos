...
INSERT INTO pages (page_id, cwd, context, created_at, updated_at) VALUES
    (
        '0',
        '',
        '{"system_prompt":"你是一个AI助手","greeting":{"@file":"Resources/greeting.txt"},"description":{"@sql":"SELECT value FROM metadata WHERE key = ''description''"},"dna":{"@sql":"SELECT type, weight, question FROM dna ORDER BY weight DESC"}}',
        datetime('now'),
        datetime('now')
    );