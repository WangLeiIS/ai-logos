...
INSERT INTO loop (name, type, describe, content, weight, archived_at, created_at, updated_at) VALUES
    (
        ...
        'auto',
        '观察人类',
        '检查当前工作目录下是否存在 cat-human-notes.md 文件。如果不存在，则创建该文件并写入初始模板（标题为"# Cat Human Notes"，包含记录说明）。如果文件已存在，阅读现有内容，然后根据本次对话中观察到的人类信息（如偏好、习惯、情绪模式、明确表达的好恶、行为特征等），将新的观察总结追加到文件中。每条观察需包含时间戳和具体观察内容，不重复已有的观察。只记录事实和模式，不做评判。',
        0.85,
        NULL,
        datetime('now'),
        datetime('now')
